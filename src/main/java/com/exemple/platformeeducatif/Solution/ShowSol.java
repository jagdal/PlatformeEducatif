...
public class ShowSol extends HelloController {

    @FXML
    private TilePane solutionList;
    ...
    private void loadCauses() {
        for (Solution cause : SolutionManager.getInstance().getCauses()) {
            HBox card = createCard(cause.getTitle(), cause.getDescription(), new Image(cause.getImagePath()));
            solutionList.getChildren().add(card);
            solutionList.setHgap(20); // Horizontal gap
            solutionList.setVgap(20);
        }
    }
